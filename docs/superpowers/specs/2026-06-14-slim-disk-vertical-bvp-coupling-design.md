...
**Coupling causality (the resolved framing).** The column provides the *closure relationship* `(Σ, T_c, f_adv) → (F, z₀, η₃, η₄)` that replaces the one-zone `Q_rad(Σ,T_c)` and `H(Σ,T_c)`. T_c stays a radial unknown; the radial **energy row keeps its form** `Q_vis − F − Q_adv = 0` (the column's emergent flux F replaces `64σT_c⁴/(3κΣ)`), so the energy balance still determines T_c — but now F comes from the vertical diffusion (decoupled from H/r) and `H = z₀` from vertical hydrostatic (not `c_s/Ω_⊥`). `f_adv` enters because the column's flux *generation* carries S11 Eq 13's advection reduction `dℱ/dz = (3𝒟/2𝒞)·(αp/(1+f_adv))·(M/r³)^½`; `f_adv` is a radial quantity, an input to the column.

```
radial state (Σ_i, T_c,i, f_adv,i) per node
   │
   ▼ for each node i (independent, warm-started):
   ┌─────────────────────────────────────────────┐
   │ (Σ,T_c,f_adv)-driven column solve (C1)       │ → F_i (emergent flux), z₀_i, T(z), ρ(z), converged_i
...
**Purpose.** Provide the column as the map `(Σ, T_c, f_adv) → (F_emergent, z₀, profile)` the radial energy row + closure need. The column is currently `T_eff`-driven (surface temperature is the input, `Σ0` an output); re-pose it so the radial-side state `(Σ, T_c)` are the inputs and the emergent flux `F` floats.
**Interface.** `solve_column_coupled(Σ, T_c, f_adv, shear, Ω_z, α, opacity, warm_start) → {F, z₀, T(z), ρ(z), P(z), converged}`, where `F = σT_eff⁴` is the emergent (top-of-column) diffusion flux and `T_c = T(0)` (midplane).
**Mechanism.** Two input swaps vs the `T_eff`-driven form: fix `Σ` (was an output `Σ0=2∫ρdz`) and fix the midplane `T_c=T(0)` (was the surface `T_eff`); free `T_eff`/`F` and the corresponding density scale. The flux generation carries the advection reduction `dℱ/dz = (3𝒟/2𝒞)·(αp/(1+f_adv))·(M/r³)^½` (S11 Eq 13), so `f_adv` (radial) is an input. F decouples from H/r because it is the *integrated radiative-diffusion* flux (set by the vertical T-gradient and opacity), and `z₀` decouples because it is the *hydrostatic* photosphere height — neither is the one-zone `c_s/Ω_⊥`. Reuses the existing column Newton + its analytic `∂R/∂U`; the change is which BCs are fixed vs free. *(The exact BC re-parametrization — `T_c`-driven as here, vs the equivalent heating-driven form with `T_c` an output and an extra consistency row — is settled in the implementation plan against the column's actual BC set; both yield the same coupled root.)*
...
**Mechanism.** Implicit-function theorem: the column satisfies `R_c(U_c; p)=0` for parameters `p=(Σ, T_c, f_adv)`; `dU_c/dp = −(∂R_c/∂U_c)⁻¹ (∂R_c/∂p)` via one back-substitution per parameter **reusing the existing LU factor**. Then `dC/dp = (∂C/∂U_c)(dU_c/dp)`, with `∂C/∂U_c` explicit (F, z₀ are state components; η from C2's profile derivative). The live radial sensitivities are `∂{F,z₀,η}/∂{Σ,T_c}` (V, ℓ enter the column only through the geometric `shear`/`Ω_z`; `f_adv`'s dependence on the radial unknowns is chained in C4). **New code:** export `∂R_c/∂p` (the residual's `Σ`/`T_c`/`f_adv` dependence — mechanical; the dependence is already in the residual).
...
- **C1 inversion robustness.** Promoting `T_eff` to an unknown with an integral BC can be stiffer than the `T_eff`-driven form; needs a good `T_eff`/`rho_mid` initial guess (seed from the one-zone `Q_rad=Q⁺` inversion).
