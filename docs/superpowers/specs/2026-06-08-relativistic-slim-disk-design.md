...
- **Radial transonic solve — hybrid bracket + relaxation (REVISED 2026-06-09).** A **two-level** method: an **outer 1-D bracket/bisection on the eigenvalue `ℓ_in`**, wrapping an **inner Newton relaxation** of the radial structure (`Σ, V, Ω, ℓ, T_c` vs `r`) on the grid `[r_s, r_out]` for a *fixed* `ℓ_in`. In the inner solve, `r_s` is the free innermost node (node 0 = the sonic point) pinned by the sonic condition `𝒟₀(r_s)=0`; the *other* regularity condition `𝒩₁(r_s)=0` is **not** imposed inside — its value `g(ℓ_in)=𝒩₁(r_s; ℓ_in)` is the outer bracketing root function. This is Sądowski's actual method (Muchotrzeb-Czerny bracketing of the regularity eigenvalue + relaxation of the structure). **Why revised from the original pure-relaxation lock:** carrying `ℓ_in` as a Newton unknown coupled to its *defining* regularity row only **indirectly** (through the whole solution — `ℓ_in` does not appear in `𝒩₁(node 0)` at all) leaves the eigenvalue and the grid-stretch-degenerate `r_s` under-pinned; the monolithic Newton froze both and the radial-momentum residual plateaued (empirically confirmed 2026-06-09, `SLIM_DIAG`). Splitting the scalar eigenvalue into a **globally-convergent 1-D bracket** (sign change → bisection always works) and leaving the structure to a *well-posed* fixed-`ℓ_in` BVP fixes this. **Still NOT through-the-saddle shooting** — only the scalar `ℓ_in` is bracketed; the rendering-critical inner structure is still solved by the well-conditioned relaxation, so the spec's original precision/2D-coupling rationale is preserved. **Seeding:** the inner relaxation starts from the Novikov-Thorne thin disk at low `Ṁ` and **continues up in `Ṁ`** to the target `f_Edd`, warm-starting both the structure and the `ℓ_in` bracket (the homotopy-continuation pattern already used to bootstrap the vertical columns). Construction time is uncapped (cached).
