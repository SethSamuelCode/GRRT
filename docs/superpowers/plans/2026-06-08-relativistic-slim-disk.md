...
### Task 5: Newton relaxation solve — free inner-node (sonic point) restructure [REVISED — option B]

**Files:** Modify `src/slim_disk_radial.cpp`, `include/grrt/scene/slim_disk_radial.h`; Test `tests/test_slim_disk_radial.cpp`.

**Why revised:** the first Task-5 attempt (commit `a6e0bde`, the WIP Newton machinery) does NOT converge. Diagnosis (opus, conclusive): with a *fixed* grid `[in.r_in, r_out]` and `r_s` *interpolated inside* it, `𝒟₀ = V²−Γ̃₁(P/Σ) < 0` at every node — the sonic point sits *below* `r_in`, off the grid, so the two regularity rows are unsatisfiable and `r_s` parks at the edge while `ℓ_in` freezes. The Newton machinery (row+column scaling for the ~33-order Jacobian, Levenberg–Marquardt for the near rank-deficient `(Σ,V)` block, state-derived scaled merit, trust region, line search, honest fallback) is sound and **carries forward unchanged** — only the grid topology, the regularity rows, and the seeding change. This is the spec §7 LOCKED design: relaxation with `(ℓ_in, r_s)` as unknowns, regularity as an interior BC, seeded by Ṁ-continuation from the thin-disk limit.

**Update 2026-06-09:** the free-inner-node restructure (changes 1–5 below) is implemented and sound, BUT the *monolithic* Newton (carrying `ℓ_in` AND `r_s` as Newton unknowns) does not converge: `ℓ_in` is coupled to its defining regularity row only indirectly (it does not appear in `𝒩₁(node 0)` at all) and `r_s` is grid-stretch-degenerate, so both freeze and the radial-momentum group plateaus (confirmed via `SLIM_DIAG`). **Fix (user-approved): the two-level HYBRID** — close `ℓ_in` by an outer 1-D bracket, not a Newton unknown (spec §7 REVISED 2026-06-09). Changes 1, 3, 4, 5 carry forward unchanged; change 2 and the solver structure become the hybrid (change 6).

1. **Grid spans `[r_s, r_out]`, `r_s = U[4N+1]` the free innermost node — node 0 *is* the sonic point.** Everywhere the log grid is built (the residual, `build_thin_disk_seed`, the solver's `rgrid`, and the unpack), set the inner edge from the *current* `r_s = U[4N+1]`, not `in.r_in`: `lr0 = log(U[4N+1])`, `lr1 = log(in.r_out)`, `r[i] = exp(lr0 + (lr1−lr0)·i/(N−1))`, so `r[0] == r_s` exactly. `in.r_in` is retained only as a hard floor/guard (`r_s` must stay `> in.r_in > horizon`). ✅ DONE.

2. **Regularity rows split between the two levels.** Node 0 carries `𝒟₀(r_s)=0` as an INNER equation (pins `r_s` for fixed `ℓ_in`); `𝒩₁(r_s)=0` is the OUTER root function (change 6), NOT an inner row. The residual function still computes both `R[4N+0]=calD0(e[0])` and `R[4N+1]=calN1(in,e[0],qadv_term_geom(0,1))` at node 0 (no interpolation), but the inner Newton uses only `R[4N+0]`; `R[4N+1]` is read by the outer bracket.

3. **Radial-momentum ODE at the sonic interval `[0,1]`: L'Hôpital.** The node-0 rhs `(𝒩₁/𝒟₀)(1−V²)` is `0/0` at convergence. Replace ONLY the node-0 rhs in the trapezoidal `[0,1]` row with `dlnV/dlnr|_0 = (d𝒩₁/dlnr)/(d𝒟₀/dlnr)·(1−V0²)` (FD across nodes 0,1, denominator floored). Node 1 keeps the direct `𝒩₁/𝒟₀`. ✅ DONE.

4. **Clean seed (no magic factors).** `slim_detail::isco_prograde(M,a)` (BPT72). Seed `r_s ← 0.98·r_isco` (clamp `> in.r_in`), node states from analytic NT. Outer-BC `T_eff` anchor uses `r_isco` not `in.r_in`. ✅ DONE.

5. **Ṁ-continuation seeding INSIDE `solve_slim_disk_radial` (spec §7).** Compute `Ṁ_Edd=10·L_Edd/c²` internally; relax first at `Ṁ_lo≈0.02·Ṁ_Edd` (reliably thin), ramp `Ṁ` geometrically (≤2× / rung, ≤12 rungs) to `in.mdot`, warm-starting each rung (structure AND the `ℓ_in` bracket) from the last converged result. Honest empty fallback if any rung fails. ✅ DONE (wraps the inner+outer solve from change 6).

6. **[NEW — the hybrid] Outer `ℓ_in` bracket around an inner fixed-`ℓ_in` relaxation.**
   - **Inner solve `relax_structure(in_rung, ℓ_in, opacity, U /*in-out, warm*/) → bool`:** Newton relaxation over the `4N+1` unknowns `{Σ_i,V_i,ℓ_i,T_c,i}×N + r_s}` — i.e. ALL of `U` *except* `U[4N]=ℓ_in`, which is held fixed at the passed value. The residual sub-system is the `4N+1` rows `{N mass, N angmom, N−1 radmom, N−1 energy, 2 outer BC, 𝒟₀(node 0)=0}` — i.e. all rows EXCEPT `R[4N+1]` (𝒩₁). Reuse ALL the existing machinery (numerical Jacobian, row+column scaling, Levenberg–Marquardt, state-derived scaled merit, trust region, line search, SLIM_DIAG) — just build the Jacobian/step over the reduced index set (skip column `4N`, skip row `4N+1`). With `ℓ_in` fixed there is no eigenvalue degeneracy: `r_s` is pinned by `𝒟₀=0` plus the radmom chain back to the outer BC, so this is a well-posed BVP that converges.
   - **Outer root `g(ℓ_in) = 𝒩₁(r_s; ℓ_in) = R[4N+1]`** after the inner solve converges. Bracket+bisect (or Brent) `g` to zero. Physical window: `ℓ_in` near `ℓ_K(r_isco)` and slightly below — scan a window (e.g. `[0.80, 1.00]·ℓ_K(r_isco)`, widen if no sign change) to find `g`'s sign change, then bisect to `tol`. If the inner solve fails to converge for a trial `ℓ_in`, treat that as the "too-high/too-low" topology and step the bracket inward (don't abort the outer loop). Warm-start the inner `U` across outer iterations.
   - **State layout note:** keep `U` length `4N+2` (`U[4N]` still stores the *current* `ℓ_in`, set by the outer loop; `U[4N+1]` is `r_s`). The residual is unchanged. Only the *Newton variable/row index sets* differ between inner (reduced) and the outer scalar bracket.

- [ ] **Step 1: Failing test** — `test_converges_midmdot`: mid-`Ṁ` (`f_Edd≈0.3`) converges at **both `a=0.9` and `a=0.998`**, `V<0` & `Σ>0` everywhere, `r_sonic < r_isco`, `𝒟₀(r_s)→0` AND `𝒩₁(r_s)→0` (both regularity conditions met — inner pins `𝒟₀`, outer drives `𝒩₁`), `f_adv` small-but-positive. `SLIM_DIAG=1` to watch the inner merit + the outer `g(ℓ_in)` bracket converge.
- [ ] **Step 2: Implement** change 6 on top of the carried 1/3/4/5. Numerical Jacobian. Honest fallback (`converged=false`, empty profile).
- [ ] **Step 3: Run → pass at both spins. Step 4: Commit** `feat(slim-disk): hybrid bracket(ℓ_in)+relaxation transonic solve (option B)`.

**Update 2026-06-10 — opacity fixed; convergence diagnosis + multi-parameter continuation.**
The opacity Rosseland-mean over-estimate (`κ_R~3.8e9` at the hot inner midplane) that was throttling `Q_rad` is FIXED (temperature-adaptive Rosseland window + H⁻ upper-T taper, `src/opacity.cpp`; `test-opacity-rosseland` green). Necessary but NOT sufficient: with physical opacity the inner relaxation still does not reach the strict floor — it descends then STALLS at merit ~0.06–0.7 (line-search `λ→1e-14`, step dies), energy + sonic-regularity groups dominant. The fingerprint (reached the root's neighbourhood, then the step collapsed) points to **finite-difference Jacobian NOISE** on the gradient-heavy terms (`Q_adv` FD gradients, the L'Hôpital sonic slope, the opacity-LUT bilinear slopes) creating a false merit minimum on the root — the same wall the column BVP hit, which is why it needed an analytic Jacobian (refinements #3).

**Strategy: diagnose basin-vs-solver, then walk a known-good solution up in BOTH spin and Ṁ (multi-parameter homotopy continuation).**
- **Phase A — diagnostic (basin or solver?).** Seed the inner solve with a NEAR-TRUE solution: at `a=0, f_Edd≈0.02` the slim disk reduces exactly to the validated Novikov–Thorne thin disk, so the NT profile there *is* essentially the answer. (i) Does `solve_slim_disk_radial` converge at `a=0, f_Edd≈0.02` at all (the easiest possible case the current tests never exercise)? (ii) Seeded with the near-true NT profile, does the inner solve reach a tight floor or stall? (iii) Direct noise probe: at the near-true point, is the FD-Jacobian Newton step downhill (line search accepts) or does it collapse `λ→0`? A rejected step AT the true solution proves Jacobian noise; also check FD-Jacobian column sensitivity to the FD step size. **Gate:** converges tight → seed/basin problem → continuation (B/C). Stalls on the true answer → solver noise → build the analytic Jacobian first.
- **Phase B — spin walk at low `f_Edd`.** Nested continuation `a: 0→0.3→0.6→0.9→0.998` at fixed low `f_Edd`, warm-starting each spin from the previous converged solution; RECORD the tightest achievable merit vs spin. Quantifies whether the noise grows faster than the solver can handle and exactly where (if anywhere) it breaks.
- **Phase C — Eddington walk.** Inner Ṁ-continuation up to `f_Edd≈0.9` at the high spins (advection-on slim-disk regime; where the radiation-pressure fold would bite a thin disk).
- **Anchor:** the `a=0, low-Ṁ` start seeds from the existing validated NT thin-disk solver (`VolumetricDisk::compute_radial_structure`) — the strongest known-good value. **Driver:** extend the in-solve Ṁ-ladder to an outer spin axis (nested spin × Ṁ), warm-starting across both.
- **Decision tree:** A-stalls → analytic Jacobian first. A-ok / B-degrades → continuation + analytic Jacobian together. A-ok / B-holds / C-reaches-0.9 → continuation alone suffices.
