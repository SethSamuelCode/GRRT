...
```
`Σ`=surface density; `V`=radial velocity (corotating frame); `Δ=r²−2Mr+a²`; `A=r⁴+r²a²+2Mra²` (=`(r²+a²)²−a²Δ`, the metric function); `𝒜` (script) = the gravitational/centrifugal term (§23), **not** the metric `A` — distinct symbols; `Ω` orbital, `Ω_K±` Keplerian; `ℓ=u_φ` specific angular momentum, `ℓ_in` its inner-edge value; `Γ` **full (radial×azimuthal) Lorentz factor** `Γ²=1/(1−V²)+ℓ²r²/A` (S11 text after Eq 23; the radial-only `1/√(1−V²)` is just `u^t` and survives alone only in the mass law `u^r`); `P=∫p dz` (vertically integrated — α-stress `W_rφ=αP`, **not** local `αp`); `Q_adv ∝ T dS/dr` (entropy advected by inflow).
...
```
(`a_*=a/M`, `r_*=r/M`.) The radial solve sets `Σ(r), T_c(r)`; **`f_adv(r)` is a column OUTPUT, not an independent input.** The vertical structure is a **two-parameter family** `(T_c, f_adv) ≡ (T_c, Σ)`, so given `(T_c, Σ)` the advected fraction `f_adv` is *determined* (S11 §3.1–3.2: solved on a grid in `(r, T_c, f_adv)`, re-indexable by `(T_c, Σ)`; numerically confirmed 2026-06-25 — freeing `f_adv` makes off-`Σ` targets converge to physical solutions). Each column returns the emergent flux, photosphere height, `f_adv`, and the pressure/density moments that feed the radial EOS coefficients → **self-consistent 2D iteration**. ⚠ Fixing `Σ`, `T_c` **and** `f_adv` simultaneously **over-determines** the column (3 constraints on a 2-parameter family) — the source of spurious "folds"; the coupled re-pose frees `f_adv` as a column unknown.
...
```
The `Q_adv` bracket coefficients (`kAdvP=η₃`, `kAdvS=1+η₃`) and the `(2πr²/Ṁη₃)` normalization are therefore now **node-local** (β-dependent), as is `Γ̃₁` in 𝒟₀ and the 𝒩₁ pressure term. The closure supplies `p_gas`,`p_mid` per node; β is clamped to `[0,1]`. The pure-gas frozen value (`η₃=3/2`, `Γ̃₁=5/3`) is recovered exactly at β=1 — this preserves the flag-#1 Q_adv-bracket correction. **NOT `η₃ = Γ₁−1`** (the inverse; that slip propagated into the §23 `Q_adv` bracket — corrected 2026-06-12, flag #1). `𝒜` = the gravitational/centrifugal term (S09 Eq 3: the `(Ω−Ω_K⁺)(Ω−Ω_K⁻)/(1−Ω̃²R̃²)` combo, `Ω̃=Ω−ω`, `ω=2Mar/A`, `R̃=A/(r²Δ^½)`). `η₃,η₄` = vertical weight functions. **`η₄ ≡ (1/Σ)∫₀^h ρz² dz`** (S11; the density second moment about the midplane — verified vs the primary source 2026-06-14). The `Ω_⊥²` in the 𝒩₁ term `Ω_⊥²(η₄/η₃)dlnη₄/dlnr` is OUTSIDE the integral; physical check: `Ω_⊥²η₄ = 2×` the specific vertical gravitational PE (since the vertical PE/area is `∫ρ·½Ω_⊥²z²dz = ½Ω_⊥²Ση₄`). One-zone: uniform-ρ ⇒ `η₄=h²/3`; Gaussian `ρ∝e^{−z²/2H²}` ⇒ `η₄=H²`. (Phase-3 / vertical-BVP coupling supplies η₄ per node; the one-zone radial model treats η-moments as constant ⇒ their gradients vanish, which is why `calN1` currently drops the η₄ term.)
...
✅ **Γ definition (#12 IMPLEMENTED 2026-06-14):** S11 (text after Eq 23) defines the full `Γ² = 1/(1−V²) + ℓ²r²/A` (radial×azimuthal Lorentz factor); S09 only says "γ is the Lorentz factor" without a formula. The code now uses this **full Γ** in the torque law (Eq 4 / row (3)) and `Q_vis` (Eq 6); the azimuthal `ℓ²r²/A` piece (where `A`=Kerr `A=r⁴+r²a²+2Mra²`, node-local `e.mech.A`) is the dominant inner-disk correction (orbital speed ≫ radial inflow). Because `Γ` now depends on ℓ, the analytic Jacobian gained `∂Γ/∂ℓ = ℓr²/(A·Γ)` and the **corrected** `∂Γ/∂V = V/((1−V²)²·Γ)` (the old `V·Γ³` is valid ONLY for the radial-only Γ). ⚠ **The mass law keeps the radial-only Γ = 1/√(1−V²)** — it is literally `u^r`, S09 Eq 1; its Jacobian row computes its OWN `Γ_rad` and uses `V·Γ_rad³`, decoupled from the now-full `e.Gamma`. **NT-band shift (slim-nt-term-probe):** `Q_vis/F_NT` moves from the inner-drooped ≈0.91–1.13 to a flatter ≈1.10–1.20 (r=3≈1.10, r=6≈1.20, r=50≈1.11). The residual ≈+15% uniform offset above Page-Thorne is the radiative angular-momentum flux S09 **drops** ("we neglected the angular momentum flux taken away by radiation" — the `2Fℓ` term PT keeps and S09 Eq 4 omits): an **intentional model difference, NOT an error**. The slim disk does not reduce exactly to Page-Thorne by construction.
