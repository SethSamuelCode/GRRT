...
- **Boundary stiffness:** the criterion switch is C⁰ but not C¹. Mitigation: the switch is *continuous* in `∇` (∇_conv→∇_rad as ∇_rad↓∇_ad), so the residual is continuous; if the Newton chatters at the boundary, add a narrow C¹ blend over `∇_rad∈[∇_ad, ∇_ad(1+ε)]` (ε~1e-3). Decide from the FD-Jacobian cross-check, not preemptively.
